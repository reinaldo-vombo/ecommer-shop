...
function MorphingDialogProvider({
   ...
   const contextValue: TContexValue = useMemo(
...
type MorphingDialogSliderProps = {
   children?: React.ReactNode;
   className?: string;
};
function MorphingDialogSlider({
   children,
   className,
}: MorphingDialogSliderProps) {
   const { uniqueId } = useMorphingDialog();

   return (
      <motion.div
         className={cn(className)}
         layoutId={`dialog-slider-${uniqueId}`}
      >
         {children}
      </motion.div>
   );
...
function MorphingDialogClose({
   ...
   return (
      <motion.button
         ...
         className={cn('absolute righ-6 top-6', className)}
         initial='initial'
         animate='animate'
         exit='exit'
         variants={variants}
      >
         {children || <ArrowLeft size={24} />}
      </motion.button>
   );
}
function MorphingDialogLogout({
   className,
   variants,
}: MorphingDialogCloseProps) {
   const { setIsOpen, uniqueId } = useMorphingDialog();
   ...
   return (
      <motion.button
         ...
         key={`dialog-log-out-${uniqueId}`}
         className={cn('absolute right-0 top-6', className)}
         initial='initial'
         animate='animate'
         exit='exit'
         variants={variants}
